...
<transition name="modal" style="display:block; width:100%">
    ...
    <slot name="body">
        ...
        <div style="text-align:left; margin-bottom:2%">
            <h1 style="font-size:25px;">Список жюри</h1>
        </div>

        <div style="overflow-y:scroll; max-height:200px; overflow-x:hidden; padding:1%">
            <div v-for="item of judgesList" class="row" :key="item.id" style="margin-bottom:1%">
                ...
                <div class="col-lg-1 col-sm-2 col-md-1 col-2">
                    ...
                    <img v-else style="margin-top:6.5px" class="img" @click="deleteJudge(item.id)" src="@/assets/times.png"/>
                </div>
            </div>
        </div>
        <div class="row justify-content-center" style="margin-top:2%">
            <div class="col-lg-4 col-12 col-sm-6">
                <b-button class="btn-passive" @click="showModal = true" :disabled="loadingStatus">Добавить жюри</b-button>
            </div>
        </div>
...
<ModalCreateJudge v-if="showModal" @close="showModal = false"/>
<ModalEditJudge :judgeId="judgeId" v-if="showModalEdit" @close="showModalEdit = false"/>
</div>
</template>
<script>
import ModalCreateJudge  from '@/components/CreateLpp/ModalCreateJudge.vue'
import ModalEditJudge  from '@/components/CreateLpp/ModalEditJudge.vue'
...
export default {
    ...
    methods:{
        async deleteJudge(judgeID){
            ...
            await this.$store.dispatch('createLpp/deleteJudge',judgeID)
